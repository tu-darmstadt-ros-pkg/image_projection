...
PeriodicImageProjection::PeriodicImageProjection(const rclcpp::Node::SharedPtr node)
  : node_(node), enabled_(false), publish_tf_(false), last_tf_stamp_(node->get_clock()->now()), image_projection_lib_(node), it_(node), always_recompute_mapping_(false)
{
...
bool PeriodicImageProjection::init()
{
  ...
  pose_param_sub_ = hector::createReconfigurableParameter(node_, "pose", std::ref(pose_vec_), "Pose of the virtual sensor in the base frame", hector::ParameterOptions<std::vector<double>>()
  ...
  connection_check_timer_ = node_->create_wall_timer
  (1s, std::bind(&PeriodicImageProjection::connectCb, this));
  return true;
}
...
void PeriodicImageProjection::projectAndPublishLatestImages()
{
  ...
  if (pixel_mapping_.empty() || always_recompute_mapping_) {
    ...
    if (image_projection_lib_.getCameraLoader().cameraInfosReceived()) {
      ...
      RCLCPP_WARN_STREAM_THROTTLE(node_->get_logger(), *(node_->get_clock()), 1, "No camera info received yet.");
      return;
    }
  }
...
bool PeriodicImageProjection::projectPixelToRayCb(const image_projection_msgs::srv::ProjectPixelTo3DRay::Request::SharedPtr req, image_projection_msgs::srv::ProjectPixelTo3DRay::Response::SharedPtr resp) {
  if (!projection_) {
    return false;
  }
