...
public:
  ...
  static void printTimeInfos()
  {
    ...
    ss << "|" << std::setw(column_width_name) << "Name " << "|" << std::setw(column_width_avg) << "Avg (ms)" << "|"
       << std::setw(column_width_max) << "Max (ms)" << "|" << std::setw(column_width_sum) << "Sum (ms)" << "|"
       << std::setw(column_width_count) << "Count" << "|" << std::endl;
    ss << line(column_width_name + column_width_avg + column_width_max + column_width_sum + column_width_count + 5)
       << std::endl;

    for (auto it = begin(Timing::timing_infos_); it != end(Timing::timing_infos_); ++it) {
      const TimingInfo& info = it->second;
      double avg = info.total_time / info.counter;
      ss << "|" << std::setw(column_width_name) << info.name << "|" << std::setw(column_width_avg) << avg << "|"
         << std::setw(column_width_max) << info.max << "|" << std::setw(column_width_sum) << info.total_time << "|"
         << std::setw(column_width_count) << info.counter << "|" << std::endl;
    }
    std::cout << ss.str();
  }
  ...
  static void printAverageTimes()
  {
    ...
    for (iter = Timing::timing_infos_.begin(); iter != Timing::timing_infos_.end(); iter++) {
      ...
      RCLCPP_INFO_STREAM(rclcpp::get_logger("Timing"),
                         "Average time '" << at.name.c_str() << "': " << avgTime << " ms [" << at.counter << "]");
    }
    if (Timing::start_counter_ != Timing::stop_counter_)
      RCLCPP_INFO_STREAM(rclcpp::get_logger("Timing"), "Problem: " << Timing::start_counter_
                                                                   << " calls of startTiming and "
                                                                   << Timing::stop_counter_ << " calls of stopTiming.");
  }
  ...
  static void printTimeSums()
  {
    ...
    if (Timing::start_counter_ != Timing::stop_counter_)
      RCLCPP_INFO_STREAM(rclcpp::get_logger("Timing"),
                         "Problem: " << Timing::start_counter_ << " calls of startTiming and " << Timing::stop_counter_
                                     << " calls of stopTiming. ");
  }
};
}  // namespace timing
